...
- June 30, 2025: Enhanced memory system with CockroachDB native vector support - migrated from mixed PostgreSQL/NeonDB to unified CockroachDB architecture, implemented CockroachDB native FLOAT[] vector storage with 1536-dimension embeddings, integrated OpenAI text-embedding-3-small with CockroachDB's built-in vector similarity operators (<-> and <=>), created high-performance semantic similarity search using native vector operations, and comprehensive distributed vector memory service
