...
export function useMediaCapture(options: MediaCaptureOptions = {}) {
  ...
  const captureFrame = useCallback((): string | null => {
    ...
    // Try alternative approach: temporarily make video visible for canvas capture
    const originalStyle = {
    ...
    try {
      // Temporarily make video fully visible for capture
      video.style.position = "fixed";
      video.style.top = "0px";
    ...
    try {
      ...
      // Hide video again after a brief moment
      setTimeout(() => {
        video.style.position = originalStyle.position;
        video.style.top = originalStyle.top;
        video.style.left = originalStyle.left;
